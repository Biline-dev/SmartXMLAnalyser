...
def main():
    ...
    mode = st.radio("Choisissez le mode :", ["Exécuter un test prédéfini", "Uploader un fichier XML personnalisé"])

    log_placeholder = st.empty()
    full_log = []

    if mode == "Exécuter un test prédéfini":
        test_cases = {
            "Test 1": {
        ...
        if st.button("🚀 Lancer ce test"):
            ...
            run_pipeline(xml_file_path, instructions, log_placeholder, full_log)

    else:
        uploaded_xml = st.file_uploader("📄 Uploader votre fichier XML", type="xml")
        uploaded_instruction = st.file_uploader("📝 Uploader votre fichier d'instructions (txt/json...)", type=["txt", "json"])

        if uploaded_xml and uploaded_instruction:
            xml_path = f"temp/{uploaded_xml.name}"
            instr_path = f"temp/{uploaded_instruction.name}"

            os.makedirs("temp", exist_ok=True)
            with open(xml_path, "wb") as f:
                f.write(uploaded_xml.read())
            with open(instr_path, "wb") as f:
                f.write(uploaded_instruction.read())

            if st.button("🚀 Lancer le traitement personnalisé"):
                run_pipeline(xml_path, instr_path, log_placeholder, full_log)


def run_pipeline(xml_file_path, instructions_path, log_placeholder, full_log):
    ...
    while should_continue:
        ...
        instructions = extract_instructions_from_file(instructions_path)
